...
interface CustomTaskNodeDesignerProps {
...
const CustomTaskNodeDesigner = React.memo(function CustomTaskNodeDesigner({
  diagramEngine,
  node: designerNode,
}: CustomTaskNodeDesignerProps): JSX.Element {
  ...
  const nodeDag = revisionState.dag?.nodes?.find((revisionNode: any) => revisionNode.nodeId === designerNode.id) ?? {};
