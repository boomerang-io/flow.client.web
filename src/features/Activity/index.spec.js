...
describe("WorkflowActivity --- RTL", () => {
  ...
  it("Filter by team", () => {
    const history = createMemoryHistory({ initialEntries: ["/"] });
    const { getByLabelText, getByText, getAllByLabelText } = rtlContextRouterRender(<WorkflowActivity {...props} history={history} />);
    ...
    expect(history.location.search).toBe("?" + queryString.stringify({ teamIds: "5e3a35ad8c222700018ccd39", ...basicQuery }));
  });

  it("Filter by workflow", async () => {
    const history = createMemoryHistory({ initialEntries: ["/"] });
    const { getByLabelText, getByText, getAllByLabelText, getAllByText, getByRole, getAllByRole } = rtlContextRouterRender(<WorkflowActivity {...props} history={history} />);
    ...
    expect(history.location.search).toBe("?" + queryString.stringify({ workflowIds: "5eb2c4085a92d80001a16d87", ...basicQuery }));
  });
