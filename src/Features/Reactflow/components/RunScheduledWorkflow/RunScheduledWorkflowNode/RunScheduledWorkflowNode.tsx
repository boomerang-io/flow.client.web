...
export default function RunScheduledWorkflowNode(props: NodeProps<WorkflowNodeData>) {
  ...
  if (mode === "executor") {
    return <RunScheduledWorkflowNodeExecution {...props} />;
  }
      ...
      >
        {({ closeModal }) => (
          <TaskUpdateModal
            ...
            currentRunScheduledWorkflowVersion={task}
            latestRunScheduledWorkflowVersion={taskTemplatesData[props.data.templateRef][0]}
          />
        )}
      </ComposedModal>
    );
...
function RunScheduledWorkflowNodeExecution(props: RunScheduledWorkflowNodeExecutionProps) {
  return <div>TODO</div>;
}
